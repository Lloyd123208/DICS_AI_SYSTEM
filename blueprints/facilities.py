...
def list_facilities():
    """Reference directory of critical infrastructure. Read access for every
    operational role; only Admin/EOC can add new entries (see can_manage_facilities)."""
...
def add_facility():
    ...
    if not permission_service.can_manage_facilities(user):
        flash('Only Admin/EOC staff can add facilities.', 'danger')
